...
function parseMarkdownToHtml(markdown: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const skipMatch = line.match(/^(?:Ticker|Tickers|TradingView|Accion|Acción|Ticket|Tickets|Poster|Thumbnail|Thumbnail_Url|Cover_Poster|Imagen_Compartido|Imagen|Image|Video|Video_Url|Date|Published_At|Fecha)\s*:\s*(.+)$/i);
    if (skipMatch) {
      if (inList) { parsedLines.push("</ul>"); inList = false; }
      if (inOrderList) { parsedLines.push("</ol>"); inOrderList = false; }
      if (inTable) { parsedLines.push("</tbody></table></div>"); inTable = false; }
