  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      ...
      >
        ...
        {/* ─── AI OPTIONS BAR (NOW ABOVE THE TEXTAREA) ─── */}
        <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-2 border-b border-neutral-100 dark:border-neutral-900 bg-neutral-50/50 dark:bg-neutral-900/50 rounded-t-2xl">
          {/* Left side actions: Attach and Canvas */}
          ...
          {/* Right side actions: Model selector dropdown (always in composer bar) */}
          <div ref={modelRef} className="relative">
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="flex items-center gap-1 rounded-lg px-2 py-1.5 text-[13px] font-semibold text-text-secondary transition-colors cursor-pointer border-0 bg-transparent"
            >
              {selectedModel.label}
            ...
            <AnimatePresence>
              {modelOpen && (
                <motion.div
                  ...
                  className="absolute bottom-full right-0 mb-1.5 w-[320px] sm:w-[340px] overflow-hidden rounded-2xl border border-border bg-surface-0 shadow-lift z-20"
                >
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            <div className="bg-gray-50 border border-gray-100 rounded-xl p-2.5 flex flex-col justify-between h-[82px]">
                              ...
                              <div className="mt-1 flex flex-col">
                                <span className="text-xs font-black text-slate-900 leading-none">{Math.max(0, 100 - (quotaData?.percentages?.daily ?? 0))}%</span>
        ...
        {/* ─── WRITING FIELD & BUTTONS ROW (NOW AT THE BOTTOM) ─── */}
        <div className="flex items-end gap-2 px-3 py-2.5">
          ...
          <textarea
            ...
            className="max-h-64 w-full resize-none bg-transparent py-2 px-1 text-[0.95rem] leading-relaxed text-text-primary outline-none focus:outline-none focus:ring-0 focus-visible:ring-0 border-0 focus:border-transparent focus-visible:outline-none placeholder:text-text-faint"
          />

          <div className="shrink-0 flex items-center gap-1.5 pb-1">
            {showMic && !isStreaming && !canSend && (
              recording ? (
                <motion.button
                  onClick={stopVoice}
            ...
            {isStreaming ? (
              ...
              (canSend || !showMic) && (
                <motion.button
                  ...
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="flex h-9 w-9 items-center justify-center rounded-full bg-brand-blue text-white shadow-glow-brand cursor-pointer border-0 disabled:opacity-40 disabled:cursor-not-allowed disabled:shadow-none"
