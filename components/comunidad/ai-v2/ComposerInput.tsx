  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      ...
      >
        {attachments.length > 0 && (
          <div className="flex flex-wrap gap-1.5 border-b border-border px-4 py-3">
            {attachments.map((a, i) => (
              <span
                key={i}
                className="group flex items-center gap-1.5 rounded-xl bg-surface-2 py-1 pl-1.5 pr-1 text-xs text-text-secondary"
              >
                {a.isImage && a.url ? (
        ...
        {/* ─── WRITING FIELD (ARRIBA) ─── */}
        <div className="flex items-start px-4 pt-4 pb-2 bg-transparent relative">
        ...
        {/* ─── BARRA DE ACCIONES Y CONTROLES (ABAJO) ─── */}
        <div className="flex items-center justify-between px-4 pb-3.5 pt-1">
          {/* Lado izquierdo: Botón de más y pills de herramientas */}
              ...
              <AnimatePresence>
                {attachOpen && (
                  <motion.div
                    ...
                    className="absolute bottom-full left-0 mb-2 w-44 overflow-hidden rounded-2xl border border-border bg-surface-0 shadow-lift z-20"
                  >
                    <button
                      role="menuitem"
          ...
          {/* Lado derecho: Selector de modelo y botón circular de acción */}
          <div className="flex items-center gap-3">
            ...
            <div ref={modelRef} className="relative">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[13px] font-semibold text-text-secondary hover:text-text-primary transition-colors cursor-pointer border-0 bg-transparent"
              >
                {selectedModel.label}
                <ChevronDown
                  className={cn(
              ...
              <AnimatePresence>
                {modelOpen && (
                  <motion.div
                    ...
                    className="absolute bottom-full right-0 mb-2 w-[320px] sm:w-[340px] overflow-hidden rounded-2xl border border-border bg-surface-0 shadow-lift z-20"
                  >
                    <div className="flex p-1 bg-gray-100 dark:bg-zinc-900 rounded-lg mx-3 mt-3 mb-2 text-xs font-semibold">
                      <button
                        type="button"
                        ...
                        className={cn(
                          ...
                          dropdownTab === "models" && "bg-white dark:bg-zinc-800 text-gray-900 dark:text-white shadow-sm"
                        )}
                      >
                        Modelos
                      </button>
                      <button
                        ...
                        className={cn(
                          "flex-1 py-1.5 rounded-md text-center transition-all cursor-pointer border-0 bg-transparent text-gray-500 font-bold",
                          dropdownTab === "limits" && "bg-white dark:bg-zinc-800 text-gray-900 dark:text-white shadow-sm"
                        )}
                      >
                        Límites de Uso
                      </button>
                      ...
                      <div className="max-h-80 overflow-y-auto py-1">
                        {availableModels.map((m) => (
                          <button
                            ...
                            className={cn(
                              "flex w-full items-center justify-between px-3.5 py-2 text-left text-sm text-text-secondary transition-colors hover:bg-surface-2 hover:text-text-primary border-0 bg-transparent cursor-pointer",
                              modelId === m.id && "bg-surface-2 font-semibold text-text-primary"
                            )}
                          >
                            ...
                            {modelId === m.id && <Check className="h-4 w-4 shrink-0 text-brand-blue" />}
                          </button>
                        ))}
                      </div>
                    ) : (
                        ...
                        {quotaData && (
                          <>
                            <div className="mb-2 text-[10px] text-text-muted">
                              Modelo: <span className="font-bold">{selectedModel.label}</span> | Plan: <span className="font-bold">{PLAN_LABELS[quotaData.plan] || quotaData.plan}</span>
                            </div>
                            <div className="grid grid-cols-3 gap-2">
                              <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-xl p-2.5 flex flex-col justify-between h-[82px]">
                                <span className="text-[9px] font-black uppercase text-gray-500 tracking-wider leading-none">Diario</span>
                                <div className="mt-1 flex flex-col">
                                  <span className="text-xs font-black text-slate-900 dark:text-white leading-none">{Math.max(0, 100 - (quotaData?.percentages?.daily ?? 0))}%</span>
                                  <span className="text-[7px] text-gray-400 mt-1 truncate">
                                    {quotaData?.resets?.daily ? `Reinicia en ${formatRemaining(quotaData.resets.daily)}` : "Restante"}
                                  </span>
                                </div>
                                <div className="h-1 w-full bg-gray-100 dark:bg-zinc-800 rounded-full mt-1.5 overflow-hidden">
                                  <div className="h-full bg-blue-500 rounded-full transition-all duration-300" style={{ width: `${Math.max(0, 100 - (quotaData?.percentages?.daily ?? 0))}%` }} />
                                </div>
                              </div>
                              ...
                              <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-xl p-2.5 flex flex-col justify-between h-[82px]">
                                ...
                                <div className="mt-1 flex flex-col">
                                  <span className="text-xs font-black text-slate-900 dark:text-white leading-none">{Math.max(0, 100 - (quotaData?.percentages?.weekly ?? 0))}%</span>
                                  <span className="text-[7px] text-gray-400 mt-1 truncate font-semibold">
                                    Restante
                                  </span>
                                </div>
                                <div className="h-1 w-full bg-gray-100 dark:bg-zinc-800 rounded-full mt-1.5 overflow-hidden">
                              ...
                              <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-xl p-2.5 flex flex-col justify-between h-[82px]">
                                ...
                                <div className="mt-1 flex flex-col">
                                  <span className="text-xs font-black text-slate-900 dark:text-white leading-none">{Math.max(0, 100 - (quotaData?.percentages?.monthly ?? 0))}%</span>
                                  <span className="text-[7px] text-gray-400 mt-1 truncate font-semibold">
                                    Restante
                                  </span>
                                </div>
                            ...
                            {quotaData?.plan !== "ultra" && (
                              <div className="px-3 pt-3">
                                <a
                                  ...
                                  className="w-full py-2 bg-slate-950 hover:bg-slate-900 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-white text-[10px] font-black rounded-lg transition-all active:scale-[0.98] cursor-pointer border-0 flex items-center justify-center gap-1.5 no-underline shadow-sm"
                                >
