  ...
  return (
    <section className="relative overflow-hidden bg-slate-50 pt-24 pb-32">
      ...
      <div className="max-w-[1400px] mx-auto px-5 lg:px-8 relative z-10">
        ...
        {/* ════ PREMIUM FEATURED TRIO (DARK THEME) ════ */}
            ...
            <StaggerItem key={course.slug}>
              <TiltCard>
                <Link href={`/cursos/${course.slug}`} className="group block no-underline h-full relative">
                  <div className="absolute -inset-0.5 bg-gradient-to-br from-[#1890FF] to-[#6366F1] rounded-[2.2rem] opacity-0 group-hover:opacity-100 transition-opacity blur-[2px]" />
                  <div className="bg-slate-900 rounded-[2rem] overflow-hidden relative h-full flex flex-col transition-all border border-slate-800 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.3)]">
                    
                    {/* Inner top glow effect */}
                    <div className="absolute top-0 inset-x-0 h-40 bg-gradient-to-b from-[#1890FF]/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity z-0 pointer-events-none" />
                    ...
                    <div className="relative overflow-hidden h-[250px] z-10 m-2 rounded-[1.5rem]">
                      ...
                      <div
                        className="absolute top-4 left-4 z-20 px-3 py-1.5 rounded-xl text-white text-[10px] uppercase tracking-wider font-bold flex items-center gap-2 backdrop-blur-md"
                    ...
                    {/* Content */}
                    <div className="p-8 flex flex-col flex-grow z-10 text-white">
                      <div className="flex gap-2 flex-wrap mb-4">
                        {course.techStack.map((tech) => (
                          <span key={tech} className="bg-slate-800 text-slate-300 px-2 py-1 rounded-md text-[10px] font-mono border border-slate-700">
                            {tech}
                          </span>
                        ))}
                      </div>
                      <h3 className="font-display text-2xl font-extrabold text-white mb-3 leading-tight group-hover:text-blue-400 transition-colors">
                        {course.title}
                      </h3>
                      ...
                      <div className="mt-auto">
                        <div className="flex items-center gap-4 text-sm text-slate-500 mb-6 pt-6 border-t border-slate-800">
                          <span className="flex items-center gap-1.5 font-medium"><Clock size={14} className="text-blue-400"/> {course.durationHours} hrs</span>
                          <span className="flex items-center gap-1.5 font-medium"><Users size={14} className="text-blue-400"/> Online Premium</span>
                        </div>
                        
                        <div className="w-full flex items-center justify-between font-bold text-white transition-all duration-300 group-hover:text-blue-400">
